...
from tangerbot_msgs.msg import DecodedVoice
from tangerbot_msgs.msg import LlmCommand  # 서비스 -> 메시지로 변경
...
class tangerbotServer(Node):
    def __init__(self):
        ...
        # 서비스 클라이언트 대신 퍼블리셔 생성
        self.publisher = self.create_publisher(LlmCommand, 'llm_command', 10)

        self.get_logger().info("🧠 LLM Node 시작됨 - '/decoded_voice' 구독 중, 'llm_command' 퍼블리시 준비됨")
    ...
    def process_text_callback(self, msg):
        ...
        if action_type is not None:
            # 서비스 호출이 아니라 메시지 퍼블리시로 변경
            pub_msg = LlmCommand()
            pub_msg.robot_id = robot_id
            pub_msg.user_id = user_id
            pub_msg.type = action_type
            pub_msg.data = str(area_number) if area_number is not None else '0'

            self.publisher.publish(pub_msg)
            self.get_logger().info(f"📨 명령 퍼블리시: {pub_msg}")
        else:
            self.get_logger().info("❌ 명령을 이해하지 못했습니다.")
